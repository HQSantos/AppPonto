...
class RegistroPontoScreen:
    ...
    def criar_formulario(self):
        ...
        Button(self.frame_formulario, text="Calcular Saldo de Horas", command=self.calcular_saldo).grid(row=10, column=0, columnspan=2, pady=5)
        campos_hora = [
            self.entry_entrada,
            ...
            try:
                ...
                if ponto.status == 'normal':
                    ...
                    self.tree.insert("", "end", values=(
                        dia.day,
                        fmt(ponto.entrada),
                        fmt(ponto.inicio_intervalo),
                        fmt(ponto.fim_intervalo),
                        fmt(ponto.saida)
                    ))
                else:
                    self.tree.insert("", "end", values=(
    ...
    def salvar(self):
        try:
            ...
            if not dia_str:
                messagebox.showwarning("Aviso", "Nenhum dia selecionado para salvar.")
                return
            dia = int(dia_str)
    ...
    def calcular_saldo(self):
        try:
            mes = self.mes
            ano = self.ano
            dias_uteis = self.get_dias_uteis(ano, mes)
            saldo_total_minutos = 0
            esperado_por_dia = 8 * 60 + 48
            ...
            for d in dias_uteis:
                try:
                    registro = RegistroPonto.objects.get(dia=d.day, mes=mes, ano=ano)
                    if registro.status != 'normal':
                        continue

                    entrada = registro.entrada or time(8, 0)
                    inicio_intervalo = registro.inicio_intervalo or time(12, 0)
                    fim_intervalo = registro.fim_intervalo or time(13, 0)
                    saida = registro.saida or time(17, 48)

                    manha_min = time_to_min(inicio_intervalo) - time_to_min(entrada)
                    tarde_min = time_to_min(saida) - time_to_min(fim_intervalo)
                    trabalhado_min = manha_min + tarde_min

                    saldo_total_minutos += (trabalhado_min - esperado_por_dia)
                except RegistroPonto.DoesNotExist:
                    continue

            if saldo_total_minutos == 0:
                mensagem = "Saldo total: 0:00 horas (zerado)."
            else:
                horas = abs(saldo_total_minutos) // 60
                mins = abs(saldo_total_minutos) % 60
                sinal = "extras" if saldo_total_minutos > 0 else "déficit"
                mensagem = f"Saldo total: {horas}:{mins:02d} horas {sinal}."

            messagebox.showinfo("Saldo de Horas", mensagem)
        except Exception as e:
            messagebox.showerror("Erro", f"Erro ao calcular saldo: {e}")